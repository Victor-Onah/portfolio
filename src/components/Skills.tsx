...
import { SiExpress, SiGit, SiMongodb, SiMongoose, SiTailwindcss } from "react-icons/si";
    ...
    return (
        <div className="">
            <div className="max-w-5xl mx-auto px-4 py-14 text-white">
                ...
                <p className="mb-4 text-gray-200">Here is a visual representation of my skills.</p>
...
const Skill = ({ name, value, icon }: { name: string; value: number; icon: React.ReactNode }) => {
    ...
    return (
        <div className="bg-blue-900 p-3 rounded-lg text-white space-y-2" key={id}>
            <div className="flex gap-2 items-center">
                <span className="text-lg">{icon}</span>
                <p className="font-semibold">{name}</p>
            </div>
            <div className="h-[3px] bg-blue-100 flex-1 rounded-full">
                <span
                    style={{ width: `${percentage}%` }}
                    ref={progressBar}
                    className="block [transition:_1s_linear_width] h-full bg-gradient-to-r from-fuchsia-600 via-blue-300 to-green-600  rounded-full"
                ></span>
            </div>
        </div>
    );
